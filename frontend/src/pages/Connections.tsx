...
interface Connection {
  ...
  package: string;
  area: string;
  status: 'pending' | 'approved' | 'installed' | 'active' | 'inactive' | 'suspended';
...
export default function Connections() {
  ...
  const filteredConnections = connections.filter(connection => {
    ...
    const customerName = connection.customer_name || '';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-[#121B2E] border border-[#232D45] rounded-xl p-6">
          ...
          <p className="text-2xl font-bold text-[#14E8B4]">{connections.filter(c => c.status === 'active').length}</p>
        </div>
        <div className="bg-[#121B2E] border border-[#232D45] rounded-xl p-6">
          <p className="text-sm text-[#8996AD] mb-2">Inactive</p>
          <p className="text-2xl font-bold text-[#5C6B85]">{connections.filter(c => c.status === 'inactive').length}</p>
        </div>
        <div className="bg-[#121B2E] border border-[#232D45] rounded-xl p-6">
          <p className="text-sm text-[#8996AD] mb-2">Suspended</p>
          <p className="text-2xl font-bold text-[#F5514B]">{connections.filter(c => c.status === 'suspended').length}</p>
        </div>
      </div>
              ...
              {filteredConnections.map((connection) => {
                ...
                return (
                  <tr key={connection.id} className="border-b border-[#232D45] hover:bg-[#1B2540]/50">
                    <td className="py-3 px-4 text-sm text-[#EAF0FB]">{connection.customer_name || 'N/A'}</td>
                    ...
                    <td className="py-3 px-4 text-sm text-[#8996AD]">{connection.installation_date ? new Date(connection.installation_date).toLocaleDateString() : 'N/A'}</td>
