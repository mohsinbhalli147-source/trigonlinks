...
import { Plus, Edit, Trash2, Search, Filter, User, Shield, DollarSign, Activity } from 'lucide-react';
...
export default function StaffAll() {
  ...
  const [filterRole, setFilterRole] = useState<'all' | 'admin' | 'staff' | 'manager' | 'technician' | 'collector' | 'sales' | 'support'>('all');
  const [filterStatus, setFilterStatus] = useState<'all' | 'active' | 'inactive' | 'suspended' | 'on-leave'>('all');
  ...
  const loadStaff = async () => {
    setLoading(true);
    setError('');
    const result = await staffApi.getAll({ limit: 100 });
    if (result.success) {
      setStaff(result.data?.data || result.data || []);
    } else {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'on-leave': return 'bg-[#F6B93B]/10 text-[#F6B93B]';
      default: return 'bg-[#5C6B85]/10 text-[#5C6B85]';
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-[#EAF0FB]">All Staff Members</h2>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-[#121B2E] border border-[#232D45] rounded-xl p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm text-[#8996AD]">Total Staff</p>
          </div>
        ...
        <div className="bg-[#121B2E] border border-[#232D45] rounded-xl p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm text-[#8996AD]">Total Salary</p>
          </div>
          <p className="text-2xl font-bold text-[#F6B93B]">Rs. {totalSalary.toLocaleString()}</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#232D45]">
                ...
                <th className="text-left py-3 px-4 text-sm font-medium text-[#8996AD]">Phone</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-[#8996AD]">Role</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-[#8996AD]">Salary</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-[#8996AD]">Hire Date</th>
                ...
                <tr key={member.id} className="border-b border-[#232D45] hover:bg-[#1B2540]/50">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-[#EAF0FB]">{member.name}</div>
                        <div className="text-sm text-[#8996AD]">{member.address}</div>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-sm text-[#8996AD]">{member.email}</td>
                  <td className="py-3 px-4 text-sm text-[#8996AD]">{member.phone}</td>
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRoleColor(member.role)}`}>
                      {member.role}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-sm text-[#EAF0FB]">Rs. {(member.salary || 0).toLocaleString()}</td>
                  <td className="py-3 px-4 text-sm text-[#8996AD]">{member.hire_date ? new Date(member.hire_date).toISOString().split('T')[0] : 'N/A'}</td>
                  <td className="py-3 px-4">
